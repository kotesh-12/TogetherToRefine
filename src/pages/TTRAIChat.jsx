...
import { exportChatAsPDF, exportMessageAsDoc, exportMessageAsPPT, exportChatAsExcel } from '../utils/exportUtils';
...
// Helper to sanitize common Mermaid syntax errors from LLM output (e.g. nested parentheses)
const cleanMermaidChart = (code) => {
    if (!code) return '';
    ...
    return lines.map(line => {
        if (!line.includes('-->') && !line.includes('==>') && !line.includes('-.->')) {
            return cleanNodeString(line);
        }
        const parts = line.split(/(\s*--(?:>|x|o)|==>|-\.-\x?\s*)/g);
        return parts.map(part => {
            if (part.includes('-') || part.includes('=')) return part;
            return cleanNodeString(part);
        }).join('');
    }).join('\n');
};
    ...
    useEffect(() => {
        if (chartRef.current && chart) {
            try {
                ...
                mermaid.render(`mermaid-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`, cleanedChart)
                    .then(({ svg }) => {
                        setSvg(svg);
                    })
                    .catch(error => {
...
export default function TTRAIChat() {
        ...
        const fetchUserProfile = async () => {
            if (user && !incognitoMode) {
                try {
                    ...
                    console.log("XP Sync: Initialization deferred (Profile table might be missing)", e);
                }
            }
        };
    ...
    const handleSend = useCallback(async (e, overrideText = null) => {
        ...
        try {
            ...
            // If running in Native App (Capacitor), use absolute URL to production
            if (Capacitor.isNativePlatform()) {
                API_URL = 'https://www.ttrai.in/api/chat';
            }
    ...
    const handleRetry = useCallback(async (index) => {
        ...
        // Truncate message history in local state to everything before this message
        const truncated = messages.slice(0, index);
        setMessages(truncated);

        // Delete this and subsequent messages in DB
        if (user && currentSessionId && !incognitoMode) {
            try {
                ...
                if (data && data.length > index) {
                    const idsToDelete = data.slice(index).map(m => m.id);
                ...
                console.error('Failed to sync retry with DB:', dbErr);
            }
        }

        // Send the message text again
        handleSend(null, originalText);
    }, [messages, user, currentSessionId, incognitoMode, handleSend]);
        ...
        >
            ...
            <div className={`chat-sidebar ${showSidebar ? 'show' : ''}`}>
                ...
                <div style={{ display: 'flex', gap: '6px', padding: '0 10px', marginBottom: '8px' }}>
                    <button onClick={() => exportChatAsPDF(messages, theme, setLoading)} style={{ flex: 1, padding: '5px', background: 'rgba(239, 68, 68, 0.1)', color: '#ef4444', border: '1px dashed rgba(239, 68, 68, 0.3)', borderRadius: '8px', cursor: 'pointer', fontSize: '10px', fontWeight: 'bold' }}>
                        📄 PDF
                    </button>
                    <button onClick={() => exportChatAsExcel(messages)} style={{ flex: 1, padding: '5px', background: 'rgba(16, 185, 129, 0.1)', color: '#10b981', border: '1px dashed rgba(16, 185, 129, 0.3)', borderRadius: '8px', cursor: 'pointer', fontSize: '10px', fontWeight: 'bold' }}>
                        📊 Excel
                    </button>
                </div>
                                ...
                                <>
                                    ...
                                    >
                                        ...
                                        if(window.confirm('Are you sure you want to delete your account and all associated data? This action cannot be undone.')) {
                                            alert('Account deletion request received. Your data will be permanently removed within 24 hours.');
                                            signOut();
                                        }
                                    }} style={{background: 'rgba(255,50,50,0.1)', color: '#ff4444', border: '1px solid rgba(255,50,50,0.3)', padding: '10px 15px', borderRadius: '8px', cursor: 'pointer', width: '100%', marginBottom: '10px', fontSize: '14px', fontWeight: 'bold'}}>Delete Account</button>
                            ...
                            <div className="header-actions-group animate-in-sideways" style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                ...
                                <button
                                    ...
                                    onClick={() => {
                                        ...
                                        const shareUrl = `${window.location.origin}/nexus/${currentSessionId}`;
                    ...
                    {messages.map((msg, i) => (
                        <AnimatedMessage key={i} msg={msg}>
                            ...
                            <div className="msg-content">
                                ...
                                <ReactMarkdown remarkPlugins={[remarkGfm, remarkMath]} rehypePlugins={[rehypeKatex]} components={customMarkdownComponents}>{msg.text}</ReactMarkdown>
                                        ...
                                        {!incognitoMode && i > 0 && (
                                            <>
                                                ...
                                                <button
                                                    ...
                                                    onClick={() => exportMessageAsPPT(msg.text, setLoading)}
                        ...
                        <div style={{ position: 'relative' }} ref={fourWayMenuRef}>
                            ...
                            {showFourWayMenu && (
                                <div style={{
                                    ...
                                    <button
                                        onClick={() => { setFourWayMode(null); setShowFourWayMenu(false); setCurrentSessionId(null); setMessages([WELCOME_MSG_CONSTANT]); setInput(''); }}
                                    ...
                                    {Object.values(FOUR_WAY_HEROES).map(mode => (
                                        <button
                                            key={mode.id}
                                            onClick={() => { setFourWayMode(mode.id); setShowFourWayMenu(false); setCurrentSessionId(null); setMessages([WELCOME_MSG_CONSTANT]); setInput(''); }}
