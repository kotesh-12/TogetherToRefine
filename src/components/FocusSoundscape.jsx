...
export const FocusSoundscape = ({ onToggle }) => {
    ...
    useEffect(() => {
        if (isActive && oscillators.current.length > 0 && audioContext.current) {
            const [oscL, oscR] = oscillators.current;
            let freqL = 432;
            let freqR = 437;
            if (mode === 'Focus') {
                freqL = 300;
            ...
            oscL.frequency.setTargetAtTime(freqL, audioContext.current.currentTime, 0.1);
            oscR.frequency.setTargetAtTime(freqR, audioContext.current.currentTime, 0.1);
        }
    }, [mode, isActive]);
