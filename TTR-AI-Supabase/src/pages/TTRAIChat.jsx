...
export default function TTRAIChat() {
    ...
    const exportMessageAsPPT = async (msgText) => {
        ...
        try {
            ...
            // Intelligent Color Themes
            const pptThemes = [
                { bg: '1A1A2E', bgAlt: 'F8FAFC', textTitle: 'FFFFFF', textBody: '475569', textDark: '1E1E2A', accent: 'A78BFA' }, // Default (Dark Purple / Light)
                { bg: '0B132B', bgAlt: 'F3F4F6', textTitle: 'FFFFFF', textBody: '374151', textDark: '111827', accent: '3B82F6' }, // Tech Blue
                { bg: '064E3B', bgAlt: 'ECFDF5', textTitle: 'FFFFFF', textBody: '065F46', textDark: '064E3B', accent: '10B981' }, // Nature Green
                { bg: '450A0A', bgAlt: 'FEF2F2', textTitle: 'FFFFFF', textBody: '7F1D1D', textDark: '450A0A', accent: 'EF4444' }, // Crimson Red
            ];
            ...
            slide1.addText('Generated by Together To Refine', { x: 1, y: 3.5, w: '100%', h: 1, fontSize: 20, color: activeTheme.accent, align: 'center' });

            const blockRegex = /Slide(?:\s*\d*)?:/gi;
            const rawBlocks = msgText.split(blockRegex).filter(b => b.trim().length > 5);
            const blocks = rawBlocks.length > 0 ? rawBlocks : msgText.split(/\n\n+/);

            blocks.forEach(block => {
                if (!block.trim() || block.length < 5) return;
                let slide = pres.addSlide();
                slide.background = { color: activeTheme.bgAlt };

                let title = "", content = "", notes = "";

                if (block.toLowerCase().includes('content:')) {
                    const contentParts = block.split(/Content:/i);
                    title = contentParts[0].replace(/[*~_`#]/g, '').trim();
                    const afterContent = contentParts[1];

                    if (afterContent.toLowerCase().includes('notes:')) {
                        const notesParts = afterContent.split(/Notes:/i);
                        content = notesParts[0].trim();
                        notes = notesParts[1].replace(/[*~_`#]/g, '').trim();
                    } else {
                        content = afterContent.trim();
                    }
                } else {
                    let lines = block.replace(/[*~_`#]/g, '').trim().split('\n');
                    title = lines[0] || '';
                ...
                if (title) {
                    ...
                    const safeQuery = encodeURIComponent(title.substring(0, 40) + " professional minimal illustration");
                ...
                if (content) {
                    ...
                    const bulletObjects = bulletLines.map(line => {
                        ...
                        return { text: text, options: { bullet: true, color: activeTheme.textBody, paraSpaceAfter: 12 } };
                    });
                ...
                if (notes) {
                    ...
                    slide.addText(title, { x: 0.5, y: 2.5, w: '4.8', h: 2, fontSize: 20, align: 'center', color: activeTheme.textBody });
                }
            });
            ...
            alert('Failed to generate visual presentation.');
        } finally {
            setLoading(false);
        }
    };
