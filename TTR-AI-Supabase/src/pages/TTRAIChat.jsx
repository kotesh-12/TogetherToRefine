...
export default function TTRAIChat() {
    ...
    const exportMessageAsPPT = async (msgText) => {
        ...
        slide1.addText('TTR AI Presentation', { x: 1, y: 1.5, w: '80%', h: 1.5, fontSize: 44, color: 'FFFFFF', align: 'center', bold: true });
        slide1.addText('Generated by Together To Refine', { x: 1, y: 3.5, w: '80%', h: 1, fontSize: 20, color: 'A78BFA', align: 'center' });
        ...
        blocks.forEach(block => {
            ...
            if (lines.length > 0) {
                slide.addText(lines[0], { x: 0.5, y: 0.5, w: '90%', h: 1, fontSize: 26, bold: true, color: '1E1E2A' });
                if (lines.length > 1) {
                    const body = lines.slice(1).join('\n');
                    slide.addText(body, { x: 0.5, y: 1.8, w: '90%', h: 4.5, fontSize: 18, color: '475569', valign: 'top' });
                } else {
                    slide.addText(lines[0], { x: 0.5, y: 2.5, w: '90%', h: 2, fontSize: 24, align: 'center', color: '475569' });
                }
            }
        });

        await pres.writeFile({ fileName: `TTR_AI_PPT_${Date.now()}.pptx` });
    };
