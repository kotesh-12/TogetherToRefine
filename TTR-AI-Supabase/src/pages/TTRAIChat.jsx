...
export default function TTRAIChat() {
    ...
    const exportMessageAsPPT = async (msgText) => {
        ...
        try {
            ...
            slide1.addText('TTR AI Presentation', { x: 1, y: 1.5, w: '100%', h: 1.5, fontSize: 44, color: 'FFFFFF', align: 'center', bold: true });
            slide1.addText('Generated by Together To Refine', { x: 1, y: 3.5, w: '100%', h: 1, fontSize: 20, color: 'A78BFA', align: 'center' });
            ...
            blocks.forEach(block => {
                ...
                let content = block.replace(/[*~_`#]/g, '').trim();
                let lines = content.split('\n');

                if (lines.length > 0) {
                    const slideTitle = lines[0];
                    // Title on the left side
                    slide.addText(slideTitle, { x: 0.5, y: 0.5, w: '4.8', h: 1, fontSize: 26, bold: true, color: '1E1E2A' });

                    // Generate AI Image dynamically based on the slide title
                    const safeQuery = encodeURIComponent(slideTitle.substring(0, 40) + " professional minimal illustration");
                    const imageUrl = `https://image.pollinations.ai/prompt/${safeQuery}?width=800&height=600&nologo=true`;

                    // Add AI image to the right side of the slide
                    slide.addImage({ path: imageUrl, x: 5.5, y: 1.0, w: 4, h: 4, sizing: { type: 'contain' } });

                    if (lines.length > 1) {
                        const body = lines.slice(1).join('\n');
                        // Text body on the left side
                        slide.addText(body, { x: 0.5, y: 1.6, w: '4.8', h: 4.5, fontSize: 16, color: '475569', valign: 'top' });
                    } else {
                        // Centered text on the left half if no body
                        slide.addText(slideTitle, { x: 0.5, y: 2.5, w: '4.8', h: 2, fontSize: 20, align: 'center', color: '475569' });
                    }
                }
            });

            await pres.writeFile({ fileName: `TTR_AI_Visual_PPT_${Date.now()}.pptx` });
        } catch (error) {
            console.error('PPTX Visual Generation Failed:', error);
    ...
    const handleSlashCommand = useCallback((command) => {
        ...
        if (command === 'ppt') prompt = "Please generate a slide-by-slide presentation about the current topic. Format it strictly as:\nSlide 1: [Title]\n\nSlide 2: [Content]\n\nSlide 3: [Content]. Make it concise and impactful.";
