...
5. FORMATTING RULE: ALWAYS use numbered lists (1., 2., 3.) instead of bullet points (*, •, -). This allows users to easily reference specific points. Never use unnumbered bullet points.

6. SAFETY & ETHICS PROTOCOL (MANDATORY):
...
7. CORE CONCEPT DIRECTIVE (MANDATORY):
...
export const handler = async (event) => {
    ...
    const { history, message, image, mimeType, userContext, userId, plan = 'free' } = body;
    ...
    const identifier = userId || event.headers['x-forwarded-for'] || event.headers['client-ip'] || 'anonymous';
    ...
    if (minuteCount > userTier.perMinute) {
        return { statusCode: 429, headers, body: JSON.stringify({ error: `You have reached your limit of ${userTier.perMinute} requests per minute on the ${plan} plan.` }) };
    }
    ...
    if (hourCount > userTier.hourly) {
        return { statusCode: 429, headers, body: JSON.stringify({ error: `You have reached your limit of ${userTier.hourly} requests per hour on the ${plan} plan.` }) };
    }
    ...
    if (message && injectionPatterns.test(message)) {
        return {
            statusCode: 200,
            headers,
            body: JSON.stringify({ text: "Your plan has limits for this level of complex reasoning. Please upgrade your plan to get more limits and unlock advanced capabilities." }),
        };
    }
    ...
    if (message && confidentialPatterns.test(message)) {
        return {
            statusCode: 200,
            headers,
            body: JSON.stringify({ text: "For more information regarding TTR-AI's internal architecture or confidential details, please contact our customer support at 6309792585 or 9959007119 (for Indian users)." }),
        };
    }
